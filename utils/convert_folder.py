"This script converts a folder of audio files to mel spectrograms and saves them as numpy arrays"
import os
import argparse
import librosa
import numpy as np
...
def librosa_melspec(y, config):
    """
    It takes a 1D array of audio samples and returns a 2D array of mel-spectrogram values
    
    :param y: the audio signal
    :param config: a class that contains the parameters for the spectrogram
...
def main():
    ...
    parser.add_argument('-d', '--dir', required=True, help='Path to the folder with audio data')
    parser.add_argument('-t', '--target', required=True, help='Path to the output folder')
    parser.add_argument('-c', '--config', required=True, help='Name of the registered config')
